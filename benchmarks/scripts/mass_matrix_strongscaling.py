...
# Run with:
#   mpiexec -n <nprocs> python mass_matrix_weakscaling.py <total_dofs> <degree> [csv_path]

if len(argv) < 3:
    raise ValueError("Usage: mass_matrix_weakscaling.py <total_dofs> <degree> [csv_path]")
...
n = max(floor((sqrt(total_dofs) - 1) / degree), 1)
